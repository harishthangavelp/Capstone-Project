...
function Rentform10() {
  ...
  const handleCheckout = async (priceId) => {
    ...
    setShowConfirmationPopup(true); // Show the confirmation popup when user clicks 'Pay'
  };
    ...
    <div className='whbgbody'>
      <div className='whrfbg'>
        <form className='whForm' action="https://formspree.io/f/mgvwqrae" method="POST">
          ...
          <button type="button" onClick={() => handleCheckout('price_1QFVbvFxddvTxBZJishnFldV')} className='bookDonewh1'>Pay</button>
        ...
        <Modal show={showConfirmationPopup} onHide={handleCloseConfirmationPopup}>
          ...
          <Modal.Footer>
            <button className="btn btn-danger" onClick={() => handleProceedToPayment('price_1QFVbvFxddvTxBZJishnFldV')}>Yes</button>
            <button className="btn btn-success" onClick={handleCloseConfirmationPopup}>No</button>
          </Modal.Footer>
        </Modal>
      </div>
    </div>
